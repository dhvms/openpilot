...
def get_dynamic_personality(v_ego, personality=log.LongitudinalPersonality.standard):
  if personality==custom.LongitudinalPersonality.relaxed:
    x_vel =  [0,    11,   14.5, 15,   20,   20.01,  25,    25.01,  36,  36.01]
    y_dist = [1.5,  1.5,  1.5,  1.6,  1.76, 1.76,   1.78,  1.78,   1.8, 1.8]
  elif personality==custom.LongitudinalPersonality.standard:
    x_vel =  [0,    11,   14.5, 15,   20,   20.01,  25,    25.01,  36,  36.01]
    y_dist = [1.40, 1.40, 1.40, 1.50, 1.60, 1.76,   1.76,  1.78,   1.8, 1.8]
  elif personality==custom.LongitudinalPersonality.moderate:
    x_vel =  [0,    11,   14.5, 15,   20,   20.01,  25,    25.01,  36,  36.01]
    y_dist = [1.3,  1.3,  1.3,  1.35, 1.35, 1.385,  1.385, 1.4,   1.4,  1.45]
  elif personality==custom.LongitudinalPersonality.aggressive:
...
def get_stopped_equivalence_factor(v_lead, v_ego, comfort_brake):
  v_diff_offset = 0
...
def desired_follow_distance(v_ego, v_lead, t_follow=None, stop_distance=STOP_DISTANCE, comfort_brake=COMFORT_BRAKE):
  if t_follow is None:
    t_follow = get_T_FOLLOW()
  return get_safe_obstacle_distance(v_ego, t_follow, stop_distance, comfort_brake) - get_stopped_equivalence_factor(v_lead, v_ego, comfort_brake)
...
class LongitudinalMpc:
  ...
  def update(self, carstate, radarstate, v_cruise, x, v, a, j, personality=log.LongitudinalPersonality.standard, dynamic_personality=False):
    v_ego = self.x0[1]
    ...
    if cruise_gap == AUTO_TR_CRUISE_GAP:
      self.t_follow = interp(carstate.vEgo, AUTO_TR_BP, AUTO_TR_V)
    else:
    ...
    lead_0_obstacle = lead_xv_0[:,0] + get_stopped_equivalence_factor(lead_xv_0[:,1], v_ego, comfort_brake)
    lead_1_obstacle = lead_xv_1[:,0] + get_stopped_equivalence_factor(lead_xv_1[:,1], v_ego, comfort_brake)
