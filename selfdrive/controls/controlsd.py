...
SR_SCALE_V = [16.3, 16.1, 15.8, 15.5, 15.3]
...
class Controls:
  def __init__(self, sm=None, pm=None, can_sock=None, CI=None):
    ...
    self.set_speed_offset = self.params.get_bool("SetSpeedOffset") * (1 if self.is_metric else CV.MPH_TO_KPH)
    ...
    self.wide_camera = TICI and self.params.get_bool('EnableWideCamera')
    self.disable_op_fcw = self.params.get_bool('DisableOpFcw')
    self.mad_mode_enabled = Params().get_bool('MadModeEnabled')
    self.dynamic_personality = Params().get_bool("DynamicPersonality")
  ...
  def publish_logs(self, CS, start_time, CC, lac_log):
    ...
    controlsState.dynamicPersonality = self.dynamic_personality
