...
static void update_model(UIState *s, 
  ...
  float max_distance = isUlimitedLength ? plan_position.getX()[TRAJECTORY_SIZE - 1] : std::clamp(plan_position.getX()[TRAJECTORY_SIZE - 1],
  ...
  update_line_data(s, plan_position, isCustomRoadUI ? pwidth : 0.9, 1.22, &scene.track_vertices, max_idx, false);
}
...
UIState::UIState(QObject *parent) : QObject(parent) {
  sm = std::make_unique<SubMaster, const std::initializer_list<const char *>>({
    ...
    "uiPlan", "gpsLocationExternal", "carControl", "liveParameters", "lateralPlan", "roadLimitSpeed", "longitudinalPlan",
  });
